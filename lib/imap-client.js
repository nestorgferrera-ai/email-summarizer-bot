...
// Busca una carpeta por nombre recorriendo el árbol de buzones, usando el
// delimitador real que reporta cada nivel (necesario para localizar
// subcarpetas tipo "INBOX.IA" o "INBOX/IA" según el servidor).
...
// Busca una carpeta por nombre; si no se encuentra recorriendo el árbol,
// prueba directamente los nombres de fallbackNames (rutas habituales).
async function findFolderByName(connection, targetName, fallbackNames = []) {
  try {
    const boxes = await connection.getBoxes();
    const found = findBoxPath(boxes, targetName, '');
...
// Mueve un mensaje (por UID) de la carpeta actualmente seleccionada a
// targetFolder. node-imap usa el comando MOVE si el servidor lo soporta, y
// si no, hace COPY + marcar \Deleted + EXPUNGE automáticamente.
async function moveMessageToFolder(connection, uid, targetFolder) {
  return new Promise((resolve, reject) => {
    connection.imap.move(uid, targetFolder, (err) => {
