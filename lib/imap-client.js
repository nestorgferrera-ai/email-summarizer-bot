...
async function searchEmails(connection, { query, field = 'text', days = null } = {}, { limit = 15, scanLimit = 200 } = {}) {
  ...
  for (const msg of candidates) {
    const from = (msg.headers?.from?.[0] || '').toLowerCase();
    const subject = (msg.headers?.subject?.[0] || '').toLowerCase();

    // Las cabeceras ya están disponibles sin I/O extra (vinieron en el search()
    // inicial); solo se paga por descargar y decodificar el cuerpo del correo
    // (messageToEmail) cuando hace falta para resolver la coincidencia o para
    // mostrar el preview de un resultado ya confirmado.
    const headerHaystack = field === 'from' ? from : field === 'subject' ? subject : `${from} ${subject}`;
    let isMatch = terms.every(term => headerHaystack.includes(term));

    if (!isMatch && field === 'text') {
      let email;
      try {
      ...
      if (terms.every(term => email.preview.toLowerCase().includes(term))) {
        matches.push(email);
